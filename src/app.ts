...
app.get("/api/v1/cases/saved/items",requireAuth,async(req:AuthRequest,res,next)=>{try{const saved=await SavedCase.find({userId:req.user!.id}).populate("caseId").sort({createdAt:-1});res.json(saved.map(s=>s.caseId).filter(Boolean))}catch(e){next(e)}});
